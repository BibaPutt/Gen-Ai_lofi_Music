...
export class PromptDjMidi extends LitElement {
  ...
  @property({ attribute: false }) public songAnalysisFn?: (songTitle: string) => Promise<string[] | null>;
  ...
  private async _handleAnalyzeSong(e: Event) {
    e.preventDefault();
    const songTitle = this._songQuery.trim();
    if (!this.songAnalysisFn || !songTitle) return;
    
    this._isAnalyzing = true;
    const newPromptTexts = await this.songAnalysisFn(songTitle);
    this._isAnalyzing = false;
    
    if (newPromptTexts) {
      this._activeMood = this._songQuery;
      this._updatePromptsFromList(newPromptTexts, 3);
    } else {
      this.dispatchEvent(new CustomEvent('error', { detail: 'Could not analyze song.' }));
    }
  }

  private _handleClearSong() {
    this._songQuery = '';
    this._handleMoodChange('Default');
  }
  ...
  private _renderTopBar() {
    ...
    <div id="top-bar">
      <div class="top-bar-section left">
        ...
        <form id="analyze-form" @submit=${this._handleAnalyzeSong}>
          <input type="text" placeholder="Analyze a song..." .value=${this._songQuery} @input=${(e: Event) => this._songQuery = (e.target as HTMLInputElement).value} ?disabled=${this._isAnalyzing}>
          <button type="submit" ?disabled=${this._isAnalyzing || !this._songQuery.trim()}>${this._isAnalyzing ? 'Analyzing...' : 'Analyze'}</button>
          <button type="button" class="secondary" @click=${this._handleClearSong} ?disabled=${this._isAnalyzing}>Clear</button>
        </form>
      </div>
